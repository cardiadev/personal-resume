// Inline arrow SVG — avoids DOM-swapping via imageToSvg() which breaks React reconciliation
const RightArrow = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 386.258 386.258"
    className="fill-current text-black w-[10px] h-[10px] absolute left-0 top-1/2 translate-y-[-50%]"
  >
    <polygon points="96.879,0 96.879,386.258 289.379,193.129" />
  </svg>
);

const knowledge = [
  { id: 1, text: "Bootstrap" },
  { id: 2, text: "Webpack, Vite" },
  { id: 3, text: "Flutter" },
  { id: 4, text: "Figma & Sketch" },
  { id: 5, text: "SCRUM" },
];
const interests = [
  { id: 1, text: "Make UI/UX Design" },
  { id: 2, text: "Create Mobile App" },
  { id: 3, text: "Site Optimization - SEO" },
  { id: 4, text: "Custom Website" },
  { id: 5, text: "Microservices" },
];

const KnowledgeInterests = () => {
  return (
    <div className="cardiadev_tm_skillbox w-full h-auto clear-both float-left pt-[90px] pr-[0px] pb-[90px] pl-[0px]">
      <div className="container">
        <div className="in w-full h-auto clear-both float-left flex">
          <div className="left w-1/2 pr-[50px]">
            <div className="cardiadev_section_title w-full h-auto clear-both float-left mb-[40px]">
              <h3 className="text-[20px] font-bold">Knowledge</h3>
            </div>
            <div className="cardiadev_tm_skill_list w-full h-auto clear-both float-left">
              <ul className="m-0 list-none">
                {knowledge.map((item) => (
                  <li className="m-0 pl-[25px] relative" key={item.id}>
                    <span>
                      <RightArrow />
                      {item.text}
                    </span>
                  </li>
                ))}
              </ul>
            </div>
          </div>
          <div className="right w-1/2 pl-[50px]">
            <div className="cardiadev_section_title w-full h-auto clear-both float-left mb-[40px]">
              <h3 className="text-[20px] font-bold">Interests</h3>
            </div>
            <div className="cardiadev_tm_skill_list w-full h-auto clear-both float-left">
              <ul className="m-0 list-none">
                {interests.map((item) => (
                  <li className="m-0 pl-[25px] relative" key={item.id}>
                    <span>
                      <RightArrow />
                      {item.text}
                    </span>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
export default KnowledgeInterests;
